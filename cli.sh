        ...
        case $choice in
            1)
                ...
                if check_tool_exists "debank_checker/main.py"; then
                    execute_tool "批量 EVM 钱包查余额" "poetry run python debank_checker/main.py"
                fi
                ;;
            2)
                # 批量钱包查余额--支持任何链
            ...
            3)
                ...
                if check_tool_exists "debank_checker/gen_used_chains.py"; then
                    execute_tool "单一 EVM 钱包查余额" "poetry run python debank_checker/used_chains_checker.py"
                fi
                ;;
            4)
                # 单一钱包查余额--支持任何链
            ...
            5)
                ...
                if check_dir_exists "evm_scavenger"; then
                    local command="nano evm_scavenger/.env && poetry run python evm_scavenger/src/gen_used_chains.py && poetry run python evm_scavenger/src/erc20.py"
            ...
            6)
                # 一键转移 EVM 上的全部原生币
                if check_dir_exists "evm_scavenger"; then
                    local command="nano evm_scavenger/.env && poetry run python evm_scavenger/src/gen_used_chains.py && poetry run python evm_scavenger/src/native.py"
            ...
            7)
                ...
                if check_dir_exists "spl_scavenger"; then
                    local command="nano spl_scavenger/.env && poetry run python spl_scavenger/spl_scavenger.py"
            ...
            8)
                ...
                if check_tool_exists "rpc_endpoint_finder/main.py"; then
                    execute_tool "获取最新的免费 RPC 端点" "poetry run python rpc_endpoint_finder/main.py"
                fi
                ;;
            9)
                # EVM 智能合约交互--调用 ABI
                if check_tool_exists "smart_contract_toolkit/erc20_nft_manager.py"; then
                    local command="nano smart_contract_toolkit/.env && poetry run python smart_contract_toolkit/erc20_nft_manager.py"
