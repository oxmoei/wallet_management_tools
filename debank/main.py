...
def choose_output_mode():
    ...
    print(colored("1️⃣ -各链和池子余额（详细模式）", "green"))
    print(colored("2️⃣ -仅总余额（简单模式）", "green"))
    print()
    while True:
        mode = input(colored("💁‍♀️  请输入 1 或 2 并回车：", "yellow", attrs=["bold"])).strip()
        if mode in ("1", "2"):
            print(colored(f"📌  你选择了模式 {mode}\n", "cyan"))
            return mode
...
def get_balances(wallets, ticker=None, output_mode="1"):
    ...
    logger.info('🔍  正在获取钱包已使用的链列表以及钱包在其中的余额...')
    ...
    logger.success(f'🔆  完成！已使用的链和池子的合计数量为: {len(chains) + len(pools)}\n')
    ...
    print()
    logger.success(f'🔆  完成！结果已生成至 {file_json}')
    logger.info(f'耗时: {round((time() - start_time) / 60, 1)} 分钟.\n')
    print("=" * 40)
...
def main():
    ...
    print(colored("📝  请输入钱包地址列表（每行一个地址，输入完后回车确认）：", "yellow", attrs=["bold"]))
    input_lines = []
    while True:
        line = input()
        if line.strip() == '':
